...
- **📒 単語帳(間違い復習)**: 間違えた問題(タイル不正解・「書けなかった」)は自動で単語帳にたまり、専用の復習モードで学習。「覚えた!」(+5XP)で単語帳から消え、「まだあやしい」(+1XP)なら残る。出題の向きも記録されるため、🇯🇵和訳モードで間違えた問題は復習でも英→日で出題される。蓄積数はホーム画面に言語別内訳つきで表示。⚙️オプションのトグルでON/OFF可能(OFF中はカード非表示・自動登録も停止。蓄積データは保持)
